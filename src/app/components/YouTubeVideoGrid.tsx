                ...
                {videos.map((video) => (
                    <div key={video.id} className="bg-white rounded-lg shadow-md overflow-hidden group cursor-pointer" onClick={() => handleVideoClick(video.id, `New words in this video`)}>
                        ...
                        <div className="p-4 group relative">
                            ...
                            <button
                                className="absolute right-4 top-1/2 transform -translate-y-1/2 px-3 py-1 bg-black text-white rounded-md flex items-center space-x-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-200 hover:bg-gray-800 hover:scale-105"
                                onClick={(e) => {
                                    e.stopPropagation();
